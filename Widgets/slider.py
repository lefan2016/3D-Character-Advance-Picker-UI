...
class CustomSlider(qg.QSlider):
    ...
    _brushBorder = QBrush(QColor(8, 10, 12))
...
#-----------------------------------------------------------------------------------------------#
    def changeColor(self, start=True):
        count = 10

        if start:
            self.glow_02 = int(count)
            self.color_at = start
            self.repaint()
...
#-----------------------------------------------------------------------------------------------------#
    ...
    def paintEvent(self,event):
        ...
        painter.drawRoundedRect(x,y+8,width,height-14,5,5)

        mid_height = (height / 2) + 1
        painter.setPen(self._pen_dark)
        painter.drawLine(2, mid_height-1,width-2,mid_height-1)
        painter.setRenderHint(qg.QPainter.Antialiasing,False)
        painter.setPen(self._pen_bright)
        painter.drawLine(2, mid_height,width-2,mid_height)
        ...
        if glowIndex > 0:
            for index, size in zip(range(4),range(12,6,-1)):
                painter.setBrush(glowBrushes[glowIndex][index])
                painter.drawEllipse(center_point,size,size)


        if self.color_at is True:
            idx = self.glow_02
            for index, size in zip(range(4),range(12,6,-1)):
                painter.setBrush(glowBrushes[idx][index])
                painter.drawEllipse(center_point,size,size)
...
#-----------------------------------------------------------------------------------------------#
class CustomSlider_glow(qg.QSlider):
    _pen_dark   = qg.QPen(qg.QColor(0 , 1, 3),1, qc.Qt.SolidLine)
    _pen_bright = qg.QPen(qg.QColor(25,26,29),1.5, qc.Qt.SolidLine)
    ...
    def __init__(self,*args,**kwargs):
        super(CustomSlider_glow, self).__init__(*args,**kwargs)
